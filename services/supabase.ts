...
const RealService = {
    ...
    getUserProfile: async (userId: string, currentUserId: string) => {
        if (!supabase) return null;
        const { data: profile } = await supabase.from('profiles').select('*, group:groups(*)').eq('id', userId).single();
        ...
        return { profile: { ...profile, is_following: !!followData }, posts: posts.filter((p: any) => p.uploader_id === userId) };
    },
    toggleLike: async (fileId: string, userId: string) => {
        ...
        if (data) { await supabase.from('likes').delete().eq('id', data.id); return false; }
        else { await supabase.from('likes').insert({ file_id: fileId, user_id: userId }); return true; }
    },
    getComments: async (fileId: string, currentUserId: string) => {
         if (!supabase) return [];
         const { data } = await supabase.from('comments').select(`*, user:profiles(*), likes:comment_likes(user_id), likes_count:comment_likes(count)`).eq('file_id', fileId).order('created_at', { ascending: true });
    ...
    deleteComment: async (id: string) => { if (supabase) await supabase.from('comments').delete().eq('id', id); },
    ...
    pinComment: async (cid: string, isPinned: boolean) => { if(supabase) await supabase.from('comments').update({ is_pinned: isPinned }).eq('id', cid); },
    ...
    updateGroup: async (id: string, updates: any) => { if(supabase) await supabase.from('groups').update(updates).eq('id', id); },
    deleteGroup: async (id: string) => { if(supabase) await supabase.from('groups').delete().eq('id', id); },
    updateUserRole: async (userId: string, role: Role) => { if(supabase) await supabase.from('profiles').update({ role }).eq('id', userId); },
    updateUserGroup: async (userId: string, groupId: string) => { if(supabase) await supabase.from('profiles').update({ group_id: groupId }).eq('id', userId); },
    deleteUser: async (userId: string) => { if(supabase) await supabase.from('profiles').delete().eq('id', userId); },
    createSubject: async (name: string, color: string, icon: string, groupId: string) => { 
        if(!supabase) return;
        const { error } = await supabase.from('subjects').insert({ name, color_hex: color, icon_name: icon, group_id: groupId }); 
        if (error) { console.error("Create Subject Error:", error); throw error; }
    },
    updateSubject: async (id: string, updates: any) => { if(supabase) await supabase.from('subjects').update(updates).eq('id', id); },
    deleteSubject: async (id: string) => { if(supabase) await supabase.from('subjects').delete().eq('id', id); },
    manageSubjectDistribution: async (newName: string, originalName: string, color: string, icon: string, targetGroupIds: string[]) => {
        ...
        if (toDeleteIds.length > 0) { await supabase.from('subjects').delete().in('id', toDeleteIds); }
        for (const gid of targetGroupIds) {
            const existing = allSubjects.find(s => s.group_id === gid);
            if (existing) { await supabase.from('subjects').update({ name: newName, color_hex: color, icon_name: icon }).eq('id', existing.id); } 
            else { await supabase.from('subjects').insert({ name: newName, color_hex: color, icon_name: icon, group_id: gid }); }
        }
    },
    getAllUsers: async () => {
        if(!supabase) return [];
    ...
    getAdminStats: async () => { 
        ...
        return { users: users || 0, groups: groups || 0, files: files || 0, storage: 'DB Local' };
    },
    ...
    updateProfile: async (userId: string, updates: any) => { if(supabase) await supabase.from('profiles').update(updates).eq('id', userId); return updates; },
    toggleFollow: async (targetId: string, myId: string) => {
        ...
        if (data) { await supabase.from('follows').delete().match({ follower_id: myId, following_id: targetId }); return false; }
        else { await supabase.from('follows').insert({ follower_id: myId, following_id: targetId }); return true; }
    },
    ...
    getUserStats: async (userId: string) => {
        ...
        let likesReceived = 0; let commentsReceived = 0; let uploadsCount = 0;
        if (files) {
            uploadsCount = files.length;
            files.forEach((f: any) => { likesReceived += f.likes?.[0]?.count || 0; commentsReceived += f.comments?.[0]?.count || 0; });
        }
    ...
    uploadProfileImage: async (userId: string, file: File, type: 'avatar' | 'background') => {
        ...
        } catch (e) { console.error("Storage Exception:", e); throw e; }
    }, 
